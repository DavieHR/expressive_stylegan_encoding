...
from .encoder import Encoder4EditingWrapper,Encoder4Editing_Encoder
...
psoe_alpha_tensor_pre = None
alpha_tensor_pre = None
...
def get_w_latents(
    ...
    for i, _path in enumerate(files):
        ...
        torch.save(latent, os.path.join(w_path, f'{i + 1}.pt'))


def get_mouth_close_w_latents(
        paths: dict,
        e4e,
        G,
        w_path=None,
    ...
    files = [os.path.join(path, x) for x in os.listdir(path)]
    files = sorted(files, key=lambda x: int(os.path.basename(x).split('.')[0]))
    close_mouth_lm = cv2.imread('/data1/chenlong/0517/video/0612/landmarks_line_n_30/1/0.jpg')
    for i, _path in enumerate(files):
        image = cv2.imread(_path)
        image[274:494, 80:-80,:] = close_mouth_lm[274:494, 80:-80,:]
...
def select_id_latent_and_s(
    ...
    for i, _path in enumerate(files):
        ...
        image_gen = cv2.cvtColor(image_gen, cv2.COLOR_RGB2BGR)
        gen_file_path = os.path.join(gen_path, f"{i}_gen.jpg")
        torch.save(style_space, os.path.join(s_path, f'{i + 1}.pt'))
...
def facial_attribute_optimization_ssltent(
    ...
    global alpha_tensor_pre
    if alpha_tensor_pre is None:
        alpha_init = [0] * 32
    else:
        alpha_init = alpha_tensor_pre
    alpha_tensor = []
    for x in alpha_init:
...
def facial_attribute_optimization(    
    ...
    global psoe_alpha_tensor_pre
    if psoe_alpha_tensor_pre is None:
        alpha_init = [0] * 32
    else:
        alpha_init = psoe_alpha_tensor_pre
    alpha_tensor = []
    for x in alpha_init:
        alpha_per_tensor = torch.tensor(x).type(torch.FloatTensor).cuda()
        alpha_per_tensor.requires_grad = True
    ...
    with torch.no_grad():
        ...
        # image_tensor_after_update_all = ss_decoder(dlatents_all)
    # image_gen = (from_tensor(image_tensor_after_update_all) * 0.5 + 0.5) * 255.0
    psoe_alpha_tensor_pre = alpha_tensor
    image_gen = None
    return dlatents_all,\
           images_tensor.detach(),\
           gt_images_tensor.detach(),\
...
def pivot_finetuning(
    ...
    def get_dataloader(
        ...
        return DataLoader(dataset, batch_size = batchsize, shuffle = False, num_workers = 0)
...
def lm_pivot_finetuning(
    ...
    from torchvision.utils import make_grid
    from torch.utils.data import DataLoader
    from .ImagesDataset import ImagesDataset_LM as ImagesDataset
    ...
    def get_dataloader(
        ...
        dataset = ImagesDataset(path_images, transforms.Compose([
            transforms.ToTensor(),
            transforms.Normalize([0.5, 0.5, 0.5], [0.5, 0.5, 0.5]),
            transforms.Resize(size=(resolution, resolution))]))
        return DataLoader(dataset, batch_size=batchsize, shuffle=False, num_workers=0)
    ...
    optim = torch.optim.Adam(e4e.parameters(), lr=lr)
    # optim = torch.optim.Adam(parameters, lr = lr)
    total_idx = 0
    epochs = kwargs.get("epochs", 10)
    writer = kwargs.get("writer", None)
    ...
    if resume_path is not None:
        epoch_from_resume = int(''.join(re.findall('[0-9]+', os.path.basename(resume_path))))
        e4e.load_state_dict(torch.load(resume_path))
        logger.info(f"resume from {epoch_from_resume}...")
        start_idx = epoch_from_resume + 1
        total_idx = epoch_from_resume * len(dataloader)
    ...
    for epoch in epoch_pbar:
        sample_loss = 0
        sample_count = 0
        for idx, (image, mouth_im) in enumerate(dataloader):
            image = image.to(device)
            mouth_im = mouth_im.to(device)
            w_tensor = e4e(mouth_im)
            image_gen = G(w_tensor)
            ret = loss_register(image, image_gen, mask, is_gradient=False)
            loss = ret['loss']
            optim.zero_grad()
            b = image_gen.shape[0]
            ...
            if idx % internal_size == 0:
                ...
                if idx == 0 and writer is not None:
                    image_to_show = torch.cat((image_gen, image, mask_to_paste_resize_tensor * image_gen + (
                                1 - mask_to_paste_resize_tensor) * image), dim=2)
                    writer.add_image(f'image', make_grid(image_to_show.detach(), normalize=True, scale_each=True),
                                     total_idx)
                if writer is not None:
                    writer.add_scalars('loss', ret, total_idx)
            ...
            if idx%10 == 0:
                ...
                mouth_im2 = mouth_im * 0.5 + 0.5
                image_gen = from_tensor(image_gen) * 255.0
                mouth_im2 = from_tensor(mouth_im2) * 255.0
                image_gen = cv2.cvtColor(image_gen, cv2.COLOR_RGB2BGR)
                mouth_im2 = cv2.cvtColor(mouth_im2, cv2.COLOR_RGB2BGR)
                mouth_im2 = cv2.resize(mouth_im2,(1024,1024))
                mouth_im_cat = np.hstack((mouth_im2, image_gen))
                gen_file_path = os.path.join('/data1/chenlong/0517/video/0612/man3_w_speed_val_4/train_imgs', f"{idx}_gen.jpg")
                cv2.imwrite(gen_file_path, mouth_im_cat)
        ...
        mouth_im2 = cv2.resize(mouth_im2, (1024, 1024))
        mouth_im_cat = np.hstack((mouth_im2, image_gen))
        gen_file_path = os.path.join('/data1/chenlong/0517/video/0612/man3_w_speed_val_4', f"1_gen.jpg")
        cv2.imwrite(gen_file_path, mouth_im_cat)
        ...
        if sample_loss < min_loss:
            lastest_model_path = os.path.join(path_snapshots, f"{epoch}.pth")
            torch.save(e4e.state_dict(), os.path.join(path_snapshots, f"{epoch}.pth"))
            min_loss = sample_loss
            logger.info(f"min_loss: {min_loss}, epoch {epoch}")
    return lastest_model_path
...
def validate_video_gen(
    ...
    face_imgs = sorted(os.listdir(face_folder_path), key=lambda x: int(x.split('.')[0].split('_')[-1]))
    ...
    with imageio.get_writer(save_video_path, fps = 25) as writer:
        for index in tqdm(range(video_length)):
            ...
            image_gt_path = os.path.join(face_folder_path, face_imgs[index])
...
def expressive_encoding_pipeline(
    ...
    mouth_close_w_path = os.path.join(save_path, "mouth_close_w_path")
    mouth_close_gen_path = os.path.join(save_path, "mouth_close_gen_path")
    ...
    os.makedirs(mouth_close_w_path, exist_ok=True)
    os.makedirs(mouth_close_gen_path, exist_ok=True)
    ...
    #               myself_e4e_path=os.path.join(os.getcwd(), "ExpressiveEncoding", "encoder4editing", "exp003",
    ...
    if pti_or_not:
        os.makedirs(w_snapshots, exist_ok=True)
        latest_w_decoder_path = w_pivot_finetuning(face_folder_path, \
                                                   w_path, \
                                                   w_snapshots, \
                                                   G, \
                                                   config_pti, \
    ...
    lm_snapshots = os.path.join(stage_four_path, "lm_snapshots")
    os.makedirs(lm_snapshots, exist_ok=True)
    lm_snapshot_files = os.listdir(lm_snapshots)

    myself_e4e_path = '/data1/chenlong/local_code/expressive_talkinghead_encoding-main/ExpressiveEncoding/third_party/models/e4e_ffhq_encode.pt'
    e4e = Encoder4Editing_Encoder(myself_e4e_path)

    epochs = 10
    pti_or_not = True
    resume_path = None
    if os.path.exists(lm_snapshots) and len(lm_snapshot_files):
        snapshot_paths = sorted(lm_snapshot_files, key=lambda x: int(x.split('.')[0]))
        latest_lm_decoder_path = os.path.join(lm_snapshots, snapshot_paths[-1])
        epoch_latest = int(''.join(re.findall('[0-9]+', snapshot_paths[-1])))
        e4e.load_state_dict(torch.load(latest_lm_decoder_path))
        for p in e4e.parameters():
            p.requires_grad = False
        pti_or_not = False
        if epoch_latest < epochs:
            pti_or_not = True
            resume_path = latest_lm_decoder_path
    if pti_or_not:
        os.makedirs(lm_snapshots, exist_ok=True)
        latest_lm_decoder_path = lm_pivot_finetuning(face_folder_path, \
                                                   G, \
                                                   lm_snapshots, \
                                                   e4e, \
                                                   config_pti, \
        ...
        logger.info(f"latest model path is {latest_lm_decoder_path}")
        e4e.load_state_dict(torch.load(latest_lm_decoder_path))
        for p in e4e.parameters():
            p.requires_grad = False

    get_mouth_close_w_latents(files_path,
                              e4e,
                              G,
                              w_path = mouth_close_w_path,
                              gen_path=mouth_close_gen_path)


    if os.path.exists(cache_path):
        gen_file_list, selected_id_image, selected_id_latent, selected_id = torch.load(cache_path)
    else:
        gen_file_list, selected_id_image, selected_id_latent, selected_id = select_id_latent_and_s(files_path,
                                                                                             G,
                                                                                             ss_decoder,
                                                                                             stage_one_path_s,
    ...
    facial_loss_register = FacialLossRegister_StyleSpace(config_facial)
    ...
    gen_length = min(len(gen_file_list), 200) if not DEBUG else 100
    ...
    for ii, _file in enumerate(pbar):
        ...
        # image_posed = 0
        # pose_param = 0
        w_with_pose = torch.load(os.path.join(mouth_close_w_path, f"{ii + 1}.pt"))
        gt_ss_latent = torch.load(os.path.join(s_path, f"{ii + 1}.pt"))
