...
def select_id_latent_and_s(
        ...
        s_path=None):
    from .loss.id_loss import IDLoss
    myself_e4e_path = '/app/pretrained_models/e4e_ffhq_encode.pt'
    e4e = Encoder4EditingWrapper(e4e_path if myself_e4e_path is None else myself_e4e_path)
...
def pti_select_id_latent_and_s_multi(
        paths: dict,
        gen_path: str,
        s_path=None,
        gpu_id=None,
...
def pose_optimization_SGD(
    ...
    resume_param = kwargs.get("resume_param", None)
    if resume_param is None:
        yaw_to_optim = torch.tensor([0.0]).type(torch.FloatTensor).to(device)#torch.from_numpy(np.array([0.0])).type(torch.FloatTensor).to("cuda")
        pitch_to_optim = torch.tensor([0.0]).type(torch.FloatTensor).to(device)#torch.from_numpy(np.array([0.0])).type(torch.FloatTensor).to("cuda")
    else:
        yaw_to_optim, pitch_to_optim = resume_param
        yaw_to_optim = yaw_to_optim.detach().reshape(1)
        pitch_to_optim = pitch_to_optim.detach().reshape(1)
    ...
    # optim = torch.optim.Adam([yaw_to_optim, pitch_to_optim], lr=kwargs.get("lr", lr))
    ...
    for epoch in range(1, epochs + 1):
        ...
        if writer is not None and DEBUG:
            ...
            writer.add_scalars(f'pose_estimate/pose', dict(yaw=yaw_to_optim, pitch=pitch_to_optim), global_step=epoch)
            # images_in_training = torch.cat(((1 - mask_to_paste_resize_tensor) * gt_tensor + gen_tensor * mask_to_paste_resize_tensor), dim = 2)
            images_in_training = torch.cat(((
                                                        1 - mask_to_paste_resize_tensor) * gt_tensor + gen_tensor * mask_to_paste_resize_tensor,
                                            mask_gt_tensor * gen_tensor, mask_gt_tensor * gt_tensor,
                                            mask_facial_tensor * gen_tensor), dim=2)
            writer.add_image(f'pose_estimate/image',
                             make_grid(images_in_training.detach(), normalize=True, scale_each=True), epoch)
        optim.step()
        sche.step()
    if VERBOSE:
        t.toc("optimize pose")
...
def pose_optimization_stage_2(
    ...
    resume_param = kwargs.get("resume_param", None)
    if resume_param is None:
        yaw_to_optim = torch.tensor([0.0,0.0]).type(torch.FloatTensor).to(device)#torch.from_numpy(np.array([0.0])).type(torch.FloatTensor).to("cuda")
        pitch_to_optim = torch.tensor([0.0,0.0]).type(torch.FloatTensor).to(device)#torch.from_numpy(np.array([0.0])).type(torch.FloatTensor).to("cuda")
    else:
        yaw_to_optim, pitch_to_optim = resume_param
        yaw_to_optim = yaw_to_optim.detach().reshape(1)
        pitch_to_optim = pitch_to_optim.detach().reshape(1)
...
def facial_attribute_optimization_ssltent(
    ...
    # logger.info(f"facial_1.1 运行时间：{elapsed_time}秒")
    ...
    end_time = time.time()
    elapsed_time = end_time - start_time
    # logger.info(f"facial_1.2 运行时间：{elapsed_time}秒")
    ...
    for epoch in range(1, epochs + 1):
        ...
        end_time = time.time()
        dlatents_tmp_elapsed_time += end_time - start_time
    ...
    # logger.info(f"facial_1.3.1 运行时间：{dlatents_tmp_elapsed_time}秒")
    # logger.info(f"facial_1.3.2 运行时间：{loss_elapsed_time}秒")

    end_time = time.time()
    elapsed_time = end_time - start_time
    # logger.info(f"facial_1.3 运行时间：{elapsed_time}秒")
...
def facial_attribute_optimization_gammas(
                                  w_latent: torch.Tensor,
                                  ground_truth: np.ndarray,
                                  ret_gt: edict,
                                  loss_register: Callable,
...
def pivot_finetuning_multi(
    ...
    lastest_model_path = pivot_train(local_rank,path_snapshots,ss_decoder,device,optim,loss_register,dataset,epochs)

    return lastest_model_path


def pivot_train(     local_rank,
                     path_snapshots: str,
                     ss_decoder: object,
                     device,
                     optim,
...
def get_pose_pipeline_batch_2_and_1(
                                 local_rank,
                                 gen_length,
                                 config_path: str,
                                 save_path: str,
    ...
    if local_rank < gpu_numbers:
        ...
        get_pose_pipeline_batch_2(local_rank, gen_length, config_path, save_path, str(gpu), start_index, end_index,
                                  path)
        ...
        logger.info(f'{start_index}:{end_index}')
        get_pose_pipeline_batch_1(local_rank, gen_length, config_path, save_path, str(gpu), start_index, end_index,
                                  path)
    torch.cuda.empty_cache()


def get_pose_pipeline_batch_1(
                                 local_rank,
                                 gen_length,
                                 config_path: str,
                                 save_path: str,
...
def get_pose_pipeline_batch_2(
    ...
    for ii, _file in enumerate(gen_file_list):
        ...
        # stage 2.
        gt_tensor, mask_gt_tensor, mask_facial_tensor = pose_optimization_stage1(
            selected_id_latent.detach(),
            np.uint8(selected_id_image),
            gen_image,
            face_info_from_gen,
        ...
        if (ii + 1) % interval == 0:
            ...
            w, gen_tensor, yaw_to_optim, pitch_to_optim = pose_optimization_stage_2(
                selected_id_latent.detach(),
                gt_tensor,
                mask_gt_tensor,
                mask_facial_tensor,
...
def get_facial_pipeline_batch_2(
                                 local_rank,
                                 gen_length,
                                 config_path: str,
                                 save_path: str,
...
def expressive_PTI_pipeline(
    ...
    logger.info(f"latest model path is {latest_decoder_path}")
    now = datetime.now()
    logger.info(f'PTI_init:{now.strftime("%Y-%m-%d %H:%M:%S")}')
    ...
    validate_video_path = os.path.join(save_path, f"validate_video_cuda_{local_rank}_20.mp4")
    # try:
    if local_rank == 0:
        validate_video_gen(
            validate_video_path,
...
def pipeline_init(
    ...
    for ii, _file in enumerate(gen_file_list):
        ...
        try:
            ...
            # torch.save(w_with_pose, os.path.join(stage_two_path, f"{ii + 1}.pt"))
            # torch.save(pose_param, os.path.join(stage_two_path, f"pose_{ii + 1}.pt"))
        ...
        gammas=facial_attribute_optimization_gammas(w_with_pose, \
                                                  gen_image, \
                                                  face_info_from_gen, \
                                                  facial_loss_register, \
                                                  ss_decoder, \
...
def pipeline_multi(
    ...
    gammas, gen_length = expressive_encoding_pipeline_init(config_path,save_path,path)
    pose_n_workers = 2 * gpu_numbers
    gpu = 0
    start_index = None
    end_index = None

    multiprocessing.spawn(get_pose_pipeline_batch_2_and_1, nprocs=pose_n_workers,
                          args=(gen_length, config_path, save_path, str(gpu), start_index, end_index, path))

    torch.cuda.empty_cache()
    processes_all_over = (len(os.listdir(stage_two_path)) != gen_length * 2)
    while (processes_all_over):
        print('wait')
        time.sleep(2)
        processes_all_over = (len(os.listdir(stage_two_path)) != gen_length * 2)
    ...
    multiprocessing.spawn(get_facial_pipeline_batch_2, nprocs=n_workers, args=(gen_length,config_path, save_path, str(gpu), start_index, end_index,path,gammas))

    os.system(f'python -m torch.distributed.launch --nproc_per_node=4 --master_port 15575 ExpressiveEncoding {config_path} {save_path} {path} PTI')
