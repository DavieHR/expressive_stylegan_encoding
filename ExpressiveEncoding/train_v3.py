...
# stylegan_path = os.path.join(WHERE_AM_I, "third_party/models/stylegan2-ffhq-512x512.pkl")
...
def pose_optimization(
    ...
    # yaw_to_optim = torch.tensor([res_gt.yaw]).type(torch.FloatTensor).to("cuda")
    # pitch_to_optim = torch.tensor([res_gt.pitch]).type(torch.FloatTensor).to("cuda")
...
def pose_optimization_w(
    ...
    resume_param = kwargs.get("resume_param", None)
    if resume_param is None:
        yaw_to_optim = torch.tensor([0.0]).type(torch.FloatTensor).to(
            "cuda")  # torch.from_numpy(np.array([0.0])).type(torch.FloatTensor).to("cuda")
        pitch_to_optim = torch.tensor([0.0]).type(torch.FloatTensor).to(
            "cuda")  # torch.from_numpy(np.array([0.0])).type(torch.FloatTensor).to("cuda")
    else:
        yaw_to_optim, pitch_to_optim = resume_param
        yaw_to_optim = yaw_to_optim.detach().reshape(1)
    ...
    for epoch in range(1, epochs + 1):
        ...
        ret = loss_register(w,  w_gt, is_gradient = False)
...
def facial_attribute_optimization_ssltent(
    ...
    gt_ss_latent_3 = [ss_latent.clone().repeat(region_num, 1).to('cuda') for ss_latent in gt_ss_latent]
...
def validate_video_gen(
    ...
    with imageio.get_writer(save_video_path, fps = 25) as writer:
        for index in tqdm(range(video_length)):
            ...
            # image_paste[274*2:494*2,80*2:-80*2,:] = image[274*2:494*2,80*2:-80*2,:]
...
def w_pivot_finetuning(
    ...
    def get_dataloader(
        ...
        dataset = ImagesDataset(path_images, path_style_latents, transforms.Compose([
            ...
            transforms.Normalize([0.5, 0.5, 0.5], [0.5, 0.5, 0.5]),
            transforms.Resize(size = (resolution, resolution))]))
        return DataLoader(dataset, batch_size = batchsize, shuffle = True, num_workers = 0)
...
def get_s_latents(
    ...
    for i, _path in enumerate(files):
        ...
        with torch.no_grad():
            latent = e4e(image_tensor)
            style_space = ss_decoder.get_style_space(latent)
        # torch.save(style_space.detach().cpu(), os.path.join(s_path, f'{i + 1}.pt'))
        torch.save([x.detach().cpu() for x in style_space], os.path.join(s_path, f'{i + 1}.pt'))
...
def expressive_encoding_pipeline(
    ...
    class PoseLossRegister_W(LossRegisterBase):
    ...
    gen_length = min(len(gen_file_list), 100) if not DEBUG else 100
    ...
    for ii, _file in enumerate(pbar):
        ...
        print(f"2. pose_optimization 运行时间：{elapsed_time}秒")
        ...
        end_time = time.time()
        elapsed_time = end_time - start_time
        print(f"3. facial_attribute_optimization 运行时间：{elapsed_time}秒")
