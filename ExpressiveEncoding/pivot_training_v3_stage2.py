...
def pivot_training(
    ...
    print(resume_path)
    if os.path.exists(resume_path) and len(resume_path):
        snapshot_files = os.listdir(resume_path)
        snapshot_paths = sorted(snapshot_files, key = lambda x: int(x.split('.')[0]))
        latest_decoder_path = os.path.join(resume_path, snapshot_paths[-1])
    print(latest_decoder_path)
    if latest_decoder_path is not None:
        decoder.load_state_dict(torch.load(latest_decoder_path), False)
    print(len(gt_path))
    ...
    validate_video_path = os.path.join(save_path, "validate_video_pti_512_ft5.mp4")
    try:
        validate_video_gen(
                            validate_video_path,
                            latest_path,
